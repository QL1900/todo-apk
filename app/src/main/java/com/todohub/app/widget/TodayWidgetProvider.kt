...
class TodayWidgetProvider : AppWidgetProvider() {
    override fun onUpdate(context: Context, mgr: AppWidgetManager, ids: IntArray) {
        for (id in ids) {
            ...
            val pi = PendingIntent.getActivity(context, id, clickIntent,
                PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
            views.setPendingIntentTemplate(R.id.widgetList, pi)
            views.setTextViewText(R.id.widgetTitle, "今日待办")
            mgr.updateAppWidget(id, views)
        }
        super.onUpdate(context, mgr, ids)
    }

    private fun toUri(id: Int): String = "todohub://widget/today/$id"
}
